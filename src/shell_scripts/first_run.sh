...
if [ $stage -le 5 ]; then
  ...
  for part in train test; do
    steps/nnet3/compute_output.sh --cmd "$cmd" --nj $nj \
      --use_gpu true --online-ivector-dir data/$part/ivectors data/$part $model exp/probs_$part
  done
fi

if [ $stage -le 6 ]; then
  cp ~/notebooks/yang_gopt/cmudict/new.txt data/local
  # Prepare lang
  local/prepare_dict.sh data/local/new.txt data/local/dict_nosp
